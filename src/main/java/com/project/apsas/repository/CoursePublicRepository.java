...
//                    " c.created_at AS createdAt, c.visibility AS visibility" +
//                    " FROM courses c" +
//                    " WHERE c.visibility='PUBLIC'" +
//                    "   AND (:kw='' OR c.name LIKE :likeKw OR c.code LIKE :likeKw OR EXISTS (" +
...
//            nativeQuery = true)
//    List<PublicCourseRow> findPublicCourses(@Param("kw") String kw,
//                                            @Param("likeKw") String likeKw,
//                                            @Param("limit") int limit,
//                                            @Param("offset") int offset);
//
//    @Query(value =
//            "SELECT COUNT(*) FROM courses c WHERE c.visibility='PUBLIC'" +
//                    " AND (:kw='' OR c.name LIKE :likeKw OR c.code LIKE :likeKw OR EXISTS (" +
//                    "     SELECT 1 FROM enrollments e JOIN users u ON u.id=e.user_id" +
//                    "     WHERE e.course_id=c.id AND e.role IN ('TEACHER','OWNER') AND u.name LIKE :likeKw))",
//            nativeQuery = true)
